...
var anyRegExp = new RegExp(`\\b(\\d+(\\.\\d+)?)px\\b`);
function Any2rem(options) {
  this.config = {};
  var val = options;
  if (val && (val = options.any) && val !== 'px') {
    anyRegExp = new RegExp(`\\b(\\d+(\\.\\d+)?)${val}\\b`);
  }
  extend(this.config, defaultConfig, options);
}

// generate @1x, @2x and @3x version stylesheet
Any2rem.prototype.generateThree = function(cssText, dpr) {
  dpr = dpr || 2;
  var self = this;
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      for (var j = 0; j < declarations.length; j++) {
        ...
        if (
          ...
          anyRegExp.test(declaration.value)
        ) {
...
Any2rem.prototype.generateRem = function(cssText) {
  var self = this;
  var config = self.config;
  var astObj = css.parse(cssText);
...
Any2rem.prototype._getCalcValue = function(type, value, dpr) {
  var config = this.config;
  var pxGlobalRegExp = new RegExp(anyRegExp.source, 'g');
